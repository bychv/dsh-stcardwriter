...
assert.match(tools, /restrict\(filter: ToolRestriction\): \(\) => void/)
assert.match(tools, /schemas\(scope\?: ScopeKey\): ToolSchema\[\]/)
assert.match(toolSchema, /type: 'json'/)

const systemPrompt = await text(dshRoot, 'packages/core/system-prompt/src/index.ts')
const agentRuntime = await text(dshRoot, 'packages/core/agent/src/runtime-types.ts')
assert.match(systemPrompt, /'system-prompt\/assemble'/)
assert.match(systemPrompt, /tools: ToolSchema\[\]/)
assert.match(agentRuntime, /export type AgentStatus = 'idle' \| 'running'/)
assert.match(agentRuntime, /'agent\/disposed'/)
