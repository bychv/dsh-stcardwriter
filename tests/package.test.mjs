...
test('bundle and client manifests target the DSH plugin loaders', async () => {
  ...
  assert.equal(pkg.version, '0.5.2-rc.1')
  ...
  assert.ok(readme.includes('已支持 DeepSeek Harness `0.1.2-rc.1`'))
  ...
  assert.match(clientSource, /stcw-preview-tabs/)
  assert.match(clientSource, /资源实时预览/)
  assert.match(clientSource, /stcw-content-input/)
  assert.match(clientSource, /已扩展为右侧整栏编辑窗/)
